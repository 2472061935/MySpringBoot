package com.zznode.common.support;

import com.zznode.common.utils.StringUtils;
...
public class StrFormatter {
	...
	public static String format(final String strPattern,final Object...argArray){
		...
		for (int argIndex = 0;argIndex < argArray.length; argIndex ++ ){
			...
			if(delimIndex == -1){
				...
				if(delimIndex > 0 && strPattern.charAt(delimIndex - 1) == C_BACKSLASH){
					if(delimIndex > 0 && strPattern.charAt(delimIndex - 1) == C_BACKSLASH){
						// 转义符前还有一个转义符，占位符依旧有效
						sbuf.append(strPattern,handlePosition,delimIndex - 1);
		...
		// append the character following the last {} pair.
		//加入最后一个占位符后所有的字符
